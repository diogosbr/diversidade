diversidade=
  function(x,table=F,ntable="diversity",base=exp(1)){
    ...
    if("xlsx" %in% rownames(installed.packages())){
      cat("Pacotes OK!")} else(install.packages("xlsx"))
    require(vegan)
    require(xlsx)
    
    final=data.frame(matrix(nrow=length(row.names(x)),ncol=4))
    colnames(final)=c("Shannon (H')","Equabilidade (J')","Espécies (S)","Abundância")
